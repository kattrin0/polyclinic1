...
const API = {
    ...
    doctors: {
        async getAll(params = {}) {
            const query = new URLSearchParams({
                paginated: 'false'  // ← ВСЕГДА загружаем всех врачей
            });
        ...
        async getActive() {
            const response = await axios.get('/doctors?active=true&paginated=false');
            return response.data;
        },
        ...
        async getByDepartment(departmentId) {
            const response = await axios.get(`/doctors?departmentId=${departmentId}&paginated=false`);
            return response.data;
        },
    ...
    services: {
        async getAll(params = {}) {
            const query = new URLSearchParams({
                paginated: 'false'  // ← ВСЕГДА загружаем все услуги
            });
            if (params.departmentId) {
                query.append('departmentId', params.departmentId);
            }
        ...
        async getByDepartment(departmentId) {
            const response = await axios.get(`/services?departmentId=${departmentId}&paginated=false`);
            return response.data;
        },
    ...
    appointments: {
        async getAll(params = {}) {
            const query = new URLSearchParams({
                page: params.page || 0,
                size: 1000,  // ← загружаем много записей
                sortBy: params.sortBy || 'id',
        ...
        async getAllWithoutPagination(params = {}) {
            const query = new URLSearchParams({
                page: 0,
                size: 5000,  // ← очень большое число
                sortBy: params.sortBy || 'id',
                sortDir: params.sortDir || 'desc'
            });
            if (params.status) query.append('status', params.status);
